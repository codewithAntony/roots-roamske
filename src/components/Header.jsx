        ...
        >
            <div className='container mx-auto px-4 flex justify-between items-center'>
                <div className='flex items-center'>
                    <h1 className={`text-2xl font-bold ${scrolled ? 'text-emerald-800' : 'text-white'}`}>Kenya
...
{['Home', 'Destination', 'Experiences', 'About', 'Contact'].map((item) => (
    <li key={item}>
<a href={`#${item.toLowerCase()}`}
className={`font-medium ${scrolled ? 'text-gray-700 hover:text-emerald-700' : 'text-white hover:text-amber-300'} transition-colors duration-300`}>{item}</a>
    </li>
))}
                    </ul>

                    <button onClick={toggleLanguage} className={`flex items-center space-x-1 font-medium ${
                        scrolled ? 'text-gray-700 hover:text-emerald-700' : 'text-white hover:text-amber-300'
                ...
                <div className='md:hidden bg-white shadow-lg absolute w-full'>
                    <ul className='pt-2 pb-4 px-4 space-y-2'>
                        {['Home', 'Destination', 'Experiences', 'About', 'Contact'].map((item) => (
                            <li key={item}>
<a href={`#${item.toLowerCase()}`}
className='block py-2 text-gray-700 hover:text-emerald-700 font-medium' onClick={() => setIsMenuOpen(false)}>
    {item}
</a>
                            </li>
                        ))}
